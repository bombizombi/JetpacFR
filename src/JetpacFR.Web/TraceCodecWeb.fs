...
module TraceCodecWeb =
  ...
  let private w8 (b: byte[]) (o: int) (v: int) =
    b.[o] <- byte (v &&& 0xFF)
  ...
  let encode (trace: Trace) : byte[] =
    ...
    for i in 0 .. 0xFFFF do
      ...
      o <- o + 1
    if o <> total then failwithf "TraceCodecWeb: wrote %d bytes, expected %d" o total
    b

  /// Parse a .jpt byte stream into a Trace (tolerates the omitted self-mod array).
  let decode (b: byte[]) : Trace =
    let mutable o = 0
    if b.Length < 34 then failwithf "trace file too short (%d bytes)" b.Length
    for i in 0 .. 3 do
      if b.[i] <> magic.[i] then failwith "not a Jetpac trace file (bad magic)"
    o <- o + 4
    let v = r16 b o
    o <- o + 2
    if v <> 1 then failwithf "unsupported trace version %d" v
    ...
    o <- o + 4
    let need = entryCount * 20 + snapCount * 32 + writeCount * 8 + portCount * 8 + frameCount * 4 + 0x10000 * 4
