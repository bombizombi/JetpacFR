...
module Dom =
    ...
    let el (tag: string) : obj =
        emitJsExpr tag "document.createElement($0)"

    let append (parent: obj) (child: obj) : obj =
        emitJsExpr (parent, child) "$0.appendChild($1)"
...
module App =
    ...
    /// Which project from games/index.json is currently open. Drives the
    /// control-file storage key and fetch path. The embedded warm boot only
    /// ships for the minimal project today; other projects stay listed but
    /// refuse to open until their web assets are deployed.
    let mutable currentProjectId: string option = None
    ...
    let heatLutR =
        heatLut |> Array.map (fun c -> byte (Convert.ToInt32(c.Substring(1, 2), 16)))

    let heatLutG =
        heatLut |> Array.map (fun c -> byte (Convert.ToInt32(c.Substring(3, 2), 16)))

    let heatLutB =
        heatLut |> Array.map (fun c -> byte (Convert.ToInt32(c.Substring(5, 2), 16)))
    ...
    let private refreshCursorLabel () =
        ...
        if n = 0 then
            cursorLabel?textContent <- "no trace yet - play the game"
        else
    ...
    and private drawGraph () =
        ...
        if List.isEmpty minedRoutines then
            ...
            graphCanvas?height <-
                40.0
                + float (columns.Values |> Seq.map (fun l -> l.Count) |> Seq.fold max 0)
                  * rowHeight
            // edges under nodes
    ...
    let private saveTrace () =
        try
            ...
            let blob: obj =
                emitJsExpr bytes "new Blob([new Uint8Array($0)], { type: 'application/octet-stream' })"
        ...
        with ex ->
            statusText?textContent <- "save failed: " + ex.Message
                        ...
                        (fun () ->
                            ...
                            try
                                ...
                                if t.Entries.Length = 0 then
                                    ...
                                    statusText?textContent <-
                                        sprintf
                                            "loaded %d instructions; mined %d routines"
                                            t.Entries.Length
                                            routines.Length
                            with ex ->
                                statusText?textContent <- "load failed: " + ex.Message)
    ...
    // ---- construction ------------------------------------------------------
    let start () =
        // The embedded warm boot ships for this project only (see Embedded.fs);
        // other launcher rows list but refuse to open until deployed.
        currentProjectId <- Some "minimal"
        ...
        Jetpac3.Core.Boot.AssetProvider <-
            fun key ->
                match key with
        ...
        | None ->
            ...
            if mem.Length = 0x10000 then
                EntryCache.save "rom" "tzx" mem Embedded.WarmState
        ...
        // toolbar wiring
        (Dom.byId "btnRun")?addEventListener (
            "click",
            fun _ ->
                pauseGame ()
                running <- true
        ...
        (Dom.byId "btnStepFrame")?addEventListener (
            "click",
            fun _ ->
                pauseGame ()
                renderFrame ()
        ...
        chkMute?addEventListener ("change", fun _ -> Audio.SetEnabled(not (unbox<bool> (chkMute?checked))))
        ...
        selSpeed?addEventListener ("change", fun _ -> cinemaSpeed <- Int32.Parse(string (selSpeed?value)))
        ...
        graphCanvas?addEventListener (
            ...
            fun (e: obj) ->
                ...
                for (entry, nx, ny) in graphNodes do
                    if hit < 0 && x >= nx && x <= nx + 104.0 && y >= ny && y <= ny + 28.0 then
                        hit <- entry

                if hit >= 0 then
                    selectRoutine hit true
        )
        ...
        graphCanvas?addEventListener (
            ...
            fun (e: obj) ->
                ...
                if hit >= 0 then
                    graphCanvas?title <- sprintf "0x%04X" hit
                else
                    graphCanvas?title <- ""
        )

        // contract/prompt tab
        (Dom.byId "btnSaveMd")?addEventListener (
            "click",
            fun _ ->
                if (string (contractText?value)).Length > 0 then
                    let blob: obj =
                        emitJsExpr (contractText?value) "new Blob([$0], { type: 'text/markdown' })"
                    ...
                    statusText?textContent <- "select a routine first"
        )

        (Dom.byId "btnCopy")?addEventListener (
            "click",
            fun _ ->
                if (string (contractText?value)).Length > 0 then
        ...
        // theater tab
        (Dom.byId "btnStage")?addEventListener (
            "click",
            fun _ ->
                match activeRoutine with
                | Some r ->
                    if (string (pasteBox?value)).Trim().Length > 0 then
                        ...
                        statusText?textContent <- "paste the model's F# first"
                | None -> statusText?textContent <- "select a routine first"
        )

        (Dom.byId "btnValidate")?addEventListener (
            "click",
            fun _ ->
                ...
                | Some r, Some _ ->
                    ...
                    if not registered then
                        theaterText?textContent <-
                            sprintf
                                "0x%04X is not covered by the lifted registry yet.\n\nStage the pasted F# (below), add it to Jetpac3.Core in Visual Studio, rebuild, then validate again."
                                r.Entry
                    else
                        let framesN = 300
        ...
        Dom.window?addEventListener (
            ...
            fun (e: obj) ->
                ...
                if isEditableTarget e then
                    ...
                    if key = " " || key.StartsWith "Arrow" then
                        e?preventDefault ()
        )
        ...
        // per-project control storage: one localStorage key per project id
        let controlKey () = "jetpacfr.control." + (defaultArg currentProjectId "minimal")
        ...
        let saveControlLocal () =
            ...
            | Some c ->
                ...
                emitJsExpr (local, controlKey (), ControlFile.toJson c) "$0.setItem($1, $2)"
                |> ignore
        ...
        let tlNiceStep (length: int) : int =
            ...
            let rec niceCeil (v: float) base10 =
                ...
                | Some n -> n * base10
                | None -> if base10 >= 1e9 then v else niceCeil v (base10 * 10.0)

            let l = float length
            let mutable step = niceCeil (l / 4.0) 1.0

            while System.Math.Floor(l / step) + 1.0 > 4.0 do
                step <- niceCeil (step * 1.01) 1.0

            while System.Math.Floor(l / step) + 1.0 < 3.0 && step > 1.0 do
                step <- niceCeil (step / 2.02) 1.0
        ...
        let tlRender () =
            ...
            let drawBrush (which: bool) ((rs, re): int * int) =
                ...
                if which then
                    chipRectA <- (chipX, chipY, chipW, chipH)
                else
                    chipRectB <- (chipX, chipY, chipW, chipH)
        ...
        let tlX (e: obj) =
            ...
            (unbox<float> e?clientX - unbox<float> rect?left)
            * (unbox<float> tlCanvas?width / unbox<float> rect?width)
        ...
        tlCanvas?addEventListener (
            ...
            fun e ->
                ...
                if inChip chipRectA x y then
                    tlChipDrag <- Some true
                elif inChip chipRectB x y then
                    tlChipDrag <- Some false
                else
                    tlChipDrag <- None
        ...
        (Dom.byId "btnBrushA")?addEventListener (
            "click",
            fun _ ->
                brushA <- true
                brushStyle ()
        ...
        (Dom.byId "btnBrushB")?addEventListener (
            "click",
            fun _ ->
                brushA <- false
                brushStyle ()
        ...
        let cmMinSpan () =
            max 32 (int (unbox<float> cmCanvas?height / 32.0))
        ...
        let cmAddrY (addr: int) =
            float (unbox<float> cmCanvas?height) * float (addr - cmViewStart)
            / float (max 1 (cmViewEnd - cmViewStart))
        ...
        let cmRender () =
            ...
            if cw > 0.0 && int cw <> int w then
                cmCanvas?width <- int cw

            if ch > 0.0 && int ch <> int h then
                cmCanvas?height <- int ch
            ...
            let m =
                JetpacFR.Core.CtrlMapModel.render
                    ...
                    comments
                    (cmInstrStarts ())
                    (session |> Option.map (fun s -> s.Memory))
                    (cmCounts ())
                    (cmSelfMod ())
                    cmViewStart
                    cmViewEnd
            ...
            let rowColor kind heat =
                ...
                if heat <= 0 then
                    sprintf "rgb(%d,%d,%d)" br bgc bb
                else
                    let (hr, hg, hb) = heatLutI[min 9 heat]
                    let t = 0.15 + 0.65 * float heat / 9.0

                    let mix x y =
                        int (Math.Round(float x + (float y - float x) * t))
            ...
            if m.Rows.Length > 0 then
                ...
                for lbl in m.Labels do
                    let y =
                        JetpacFR.Core.CtrlMapModel.addrY m.ViewStart m.ViewEnd m.Rows.Length lbl.Addr
        ...
        // ---- launcher (ZX Spectrum Game Changer) + names tab + mode bar -------
        // Web parity with the desktop shell: a startup project picker over the
        // static games/index.json (hosting cannot list directories), a back
        // button in the toolbar, the recording/replay mode line under the
        // screen, and a Names tab over the loaded control file.
        let fetchProjects (cont: obj -> unit) : unit =
            let p: obj =
                emitJsExpr
                    ()
                    "fetch('games/index.json').then(function (r) { return r.ok ? r.json() : null }).catch(function () { return null })"

            emitJsExpr (p, cont) "$0.then($1)" |> ignore

        let mutable projects: obj = null
        let mutable selectedIdx = -1

        let projAt (i: int) : obj = emitJsExpr (projects, i) "$0[$1]"

        /// openSelected is wired after openProject exists; row double-clicks
        /// call it at click time.
        let mutable openSelected: unit -> unit = fun () -> ()

        let rec renderLauncher () =
            let list = Dom.byId "launcherList"
            list?innerHTML <- ""
            ...
            if isNull projects then
                ...
                row?textContent <- "games/index.json not found - the launcher needs a projects index"

                Dom.append list row |> ignore
            else
                let n: int = unbox projects?length

                for i in 0 .. n - 1 do
                    let g = projAt i
                    let id: string = string g?id
                    let name: string = string g?name
                    let isWeb: bool = unbox g?web
                    // "default" is an F# keyword, so ?-access cannot spell it
                    let isDef: bool = unbox (emitJsExpr g "$0.default")
                    let row = Dom.el "div"
                    ...
                    let title = Dom.el "div"
                    title?className <- "proj-title"

                    title?textContent <-
                        sprintf "%s (%s)%s" name id (if isDef then "  - auto-load default" else "")

                    Dom.append row title |> ignore
                    let chips = Dom.el "div"
                    ...
                    chip
                        (if unbox g?trace then "chip-on" else "chip-off")
                        (if unbox g?trace then "trace: yes" else "trace: none")

                    chip
                        (if unbox g?keys then "chip-on" else "chip-off")
                        (if unbox g?keys then "key script: yes" else "key script: no")

                    chip
                        (if unbox g?control then "chip-on" else "chip-off")
                        (if unbox g?control then "control file: yes" else "control file: no")

                    chip
                        (if unbox g?ce then "chip-on" else "chip-off")
                        (if unbox g?ce then "CE program: yes" else "CE program: no")

                    chip "chip-on" (sprintf "boot: %s" (string g?boot))
                    Dom.append row chips |> ignore
                    ...
                    row?addEventListener ("dblclick", fun _ -> openSelected ())
                    Dom.append list row |> ignore

        // names tab: the whole control file at a glance (blocks + comments;
        // the web control model has no named-symbol table yet). Clicking a
        // row jumps when the address executed in the trace.
        let refreshNamesList () =
            let list = Dom.byId "namesList"
            let count = Dom.byId "namesCount"
            list?innerHTML <- ""

            let mkRow (text: string) (color: string) (addr: int) =
                let row = Dom.el "div"
                    ...
                    fun _ ->
                        if addr < 0 then
                            ...
                            | Some t when
                                ...
                                refreshAll ()
                                syncSlider ()
                                statusText?textContent <- sprintf "jumped to 0x%04X" addr
                            | _ ->
                                statusText?textContent <-
                                    sprintf "0x%04X has not executed in this trace" addr
                )
            ...
            match control with
            | None ->
                mkRow "no control file loaded - use New / Imp in the control map" dim -1
                count?textContent <- ""
            | Some c ->
                for b in c.Blocks do
                    mkRow (sprintf "block    %04X..%04X  %s" b.Start b.EndExcl b.Name) cyan b.Start
                ...
                for m in c.Comments do
                    ...
                    mkRow (sprintf "%-7s %s  %s" (ControlFile.kindToString m.Kind) span m.Text) color m.Addr

                count?textContent <-
                    sprintf "%d blocks, %d comments" c.Blocks.Length c.Comments.Length

        let closeLauncher () =
            (Dom.byId "launcher")?style?display <- "none"

            if currentProjectId.IsSome then
                (Dom.byId "btnMenu")?style?display <- "inline-block"

        let openProject (g: obj) : unit =
            let id: string = string g?id
            let name: string = string g?name
            let isWeb: bool = unbox g?web
            ...
            if not isWeb then
                statusText?textContent <-
                    sprintf
                        "%s is not deployed to the web build yet - copy its web assets into webroot/games/%s and regenerate games/index.json"
                        name
                        id
            else
                let switch =
                    match currentProjectId with
                    | Some c -> c <> id
                    | None -> true
                ...
                if switch then
                    // Per-project control file: forget the previous project's
                    // copy, then load the new project's (browser storage
                    // first, then the served games/<id>/control.json).
                    control <- None
                    let local: obj = Dom.window?localStorage

                    match (try emitJsExpr (local, controlKey ()) "$0.getItem($1)" with _ -> null) with
                    | null ->
                        let path = sprintf "games/%s/control.json" id

                        let p: obj =
                            emitJsExpr
                                path
                                "fetch($0).then(function (r) { return r.ok ? r.text() : null }).catch(function () { return null })"
                        ...
                        let apply (txt: obj) =
                            if not (isNull txt) then
                                try
                                    ...
                                    cmRender ()
                                    refreshNamesList ()
                                    statusText?textContent <- sprintf "control file loaded from %s" path
                                with ex ->
                                    statusText?textContent <- "stored control.json ignored: " + ex.Message

                        emitJsExpr (p, apply) "$0.then($1)" |> ignore
                    | stored ->
                        try
                            ...
                            refreshNamesList ()
                        with ex ->
                            statusText?textContent <- "stored control ignored: " + ex.Message

                    statusText?textContent <- sprintf "%s opened" name

        openSelected <-
            fun () ->
                if selectedIdx >= 0 then
                    openProject (projAt selectedIdx)

        let showLauncher () =
            (Dom.byId "launcher")?style?display <- "flex"
            (Dom.byId "btnMenu")?style?display <- "none"
            statusText?textContent <- "main menu"

            fetchProjects (fun list ->
                projects <- list

                // preselect: the auto-load default when it is web-deployed,
                // else the first deployed project, else the first row
                selectedIdx <- -1

                if not (isNull projects) then
                    let n: int = unbox projects?length
                    let mutable defIdx = -1
                    let mutable webIdx = -1

                    for i in 0 .. n - 1 do
                        let g = projAt i

                        // "default" is an F# keyword: access it via JS
                        if unbox (emitJsExpr g "$0.default") then
                            defIdx <- i

                        if webIdx = -1 && unbox g?web then
                            webIdx <- i

                    selectedIdx <-
                        if defIdx >= 0 && unbox (projAt defIdx)?web then
                            defIdx
                        elif webIdx >= 0 then
                            webIdx
                        elif defIdx >= 0 then
        ...
        (Dom.byId "btnLauncherOpen")
            ?addEventListener ("click", fun _ -> openSelected ())

        (Dom.byId "btnLauncherRescan")
            ?addEventListener ("click", fun _ -> showLauncher ())
        ...
        (Dom.byId "btnNamesRefresh")
            ?addEventListener ("click", fun _ -> refreshNamesList ())

        // the static tab chrome in index.html switches panels; refresh the
        // names list whenever the tab strip is clicked so it never shows a
        // previous project's rows
        (Dom.byId "tabbar")
            ?addEventListener ("click", fun _ -> refreshNamesList ())

        // mode line under the screen (desktop parity): recording vs replaying
        let updateModeBar () =
            let bar = Dom.byId "modeBar"

            match session with
            | Some _ ->
                let recOn: bool = unbox chkRec?checked
            ...
            fun e ->
                ...
                | Some _ ->
                        ...
                        fun _ ->
                            ...
                            | Some c ->
                                ...
                                if canSplit then
                                    ...
                                    statusText?textContent <- sprintf "%04X is not an instruction start" addr
                            | None -> ()
                    ...
                    addItem "rename block..." (fun c ->
                        ...
                        if isNull (box name) || name.Trim().Length = 0 then
                            c
                        else
                            ControlFile.renameBlockAt c addr (name.Trim()))

                    addItem "line comment..." (fun c ->
                        let txt: string = unbox (Dom.window?prompt (sprintf "comment at %04X" addr, ""))

                        if isNull (box txt) then
                            c
                        else
                            ControlFile.upsert
                                c
        ...
        Dom.window?addEventListener (
            ...
            fun e ->
                ...
                if
                    menu?style?display = "block"
                    && not (isNull e?target)
                    && not (obj.ReferenceEquals(e?target, menu))
                then
                    let inside: bool =
                        emitJsExpr (e?target, menu) "$0.closest && $0.closest('#ctrlMenu') != null"
        ...
        Dom.setTimeout
            (fun () ->
                try
                    session <-
                        Some(TraceSession(Embedded.decode Embedded.RomB64, Embedded.decode Embedded.TzxB64, capacity))

                    startGame ()
                    refreshAll ()
                    syncSlider ()
                with ex ->
                    statusText?textContent <- "boot failed: " + ex.Message)
            30

        // keep the timeline extent in step with the recording, and the mode
        // line in step with the session state
        Dom.setInterval
            (fun () ->
                updateModeBar ()
        ...
        // the launcher opens on top of the (instantly warm-booted) emulator,
        // mirroring the desktop: Open closes it onto the chosen project
        showLauncher ()
        ...
        (Dom.byId "btnCtrlNew")?addEventListener (
            "click",
            fun _ ->
        ...
        (Dom.byId "btnCtrlSave")?addEventListener (
            "click",
            fun _ ->
                match control with
                | Some c when c.Dirty ->
        ...
        (Dom.byId "btnCtrlDl")?addEventListener (
            "click",
            fun _ ->
                match control with
                | Some c ->
        ...
        (Dom.byId "btnCtrlImport")?addEventListener (
            "click",
            fun _ -> emitJsExpr () "document.getElementById('ctrlFileInput').click()" |> ignore
        )

        (Dom.byId "ctrlFileInput")?addEventListener (
            "change",
            fun e ->
                ...
                if not (isNull file) then
                    ...
                    reader?onload <-
                        fun _ ->
                            ...
                            with ex ->
                                statusText?textContent <- "import failed: " + ex.Message
        ...
        let applyFetched (txt: obj) =
            if not (isNull txt) && control.IsNone then
                try
                    control <- Some(ControlFile.fromJson (string txt))
                    cmRender ()
                    statusText?textContent <- sprintf "control file loaded from games/%s/control.json" (defaultArg currentProjectId "minimal")
                with ex ->
                    statusText?textContent <- "stored control.json ignored: " + ex.Message

        match
            (try
                emitJsExpr (local, controlKey ()) "$0.getItem($1)"
             with _ ->
                 null)
        with
        | null ->
            let path = sprintf "games/%s/control.json" (defaultArg currentProjectId "minimal")
