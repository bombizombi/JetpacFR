...
module Boot =
    ...
    let mutable AssetProvider : string -> byte[] = fun key -> failwithf "Boot.AssetProvider not set for %s" key
    ...
    let bootToEntry (romKey: string) (tzxKey: string) (onFrame: (byte[] * int -> unit) option) : Jetpac.Core.Spectrum48 * int64 =
        let rom = AssetProvider romKey
        let tzx = AssetProvider tzxKey
        let spans = findCodeSpans tzx
        if spans.IsEmpty then failwith "TZX contains no CODE blocks - cannot auto-boot"
        ...
        let spanLoaded (start: int) (expected: byte[]) : bool =
            ...
            while ok && i < expected.Length do
                if spec.Memory[start + i] <> expected[i] then ok <- false
                i <- i + 1
            ok
