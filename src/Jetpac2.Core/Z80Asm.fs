...
type Z80Builder() =
    ...
    member _.While(cond: unit -> bool, body: unit -> Z80Op list) =
        ...
        if guard >= 1_000_000 then
            invalidOp "Z80 DSL While exceeded 1M unrolled iterations"
...
module Z80 =
    ...
    let private lblOp
        ...
        { Mnemonic = mnemonic
          ...
          Encode =
            Some(fun addr ->
                if !c < 0 then
                    failwithf "unresolved label for %s" mnemonic

                encode addr) }

    let JP_LBL (c: int ref) =
        lblOp "JP label" [| 0xC3uy; 0x00uy; 0x00uy |] (runOf Z80Table.main 0xC3) c (fun _ ->
            [| 0xC3uy; byte (!c &&& 0xFF); byte ((!c >>> 8) &&& 0xFF) |])

    let JP_NZ_LBL (c: int ref) =
        lblOp "JP NZ,label" [| 0xC2uy; 0x00uy; 0x00uy |] (runOf Z80Table.main 0xC2) c (fun _ ->
            [| 0xC2uy; byte (!c &&& 0xFF); byte ((!c >>> 8) &&& 0xFF) |])

    let JP_Z_LBL (c: int ref) =
        lblOp "JP Z,label" [| 0xCAuy; 0x00uy; 0x00uy |] (runOf Z80Table.main 0xCA) c (fun _ ->
            [| 0xCAuy; byte (!c &&& 0xFF); byte ((!c >>> 8) &&& 0xFF) |])

    let JP_NC_LBL (c: int ref) =
        lblOp "JP NC,label" [| 0xD2uy; 0x00uy; 0x00uy |] (runOf Z80Table.main 0xD2) c (fun _ ->
            [| 0xD2uy; byte (!c &&& 0xFF); byte ((!c >>> 8) &&& 0xFF) |])

    let JP_C_LBL (c: int ref) =
        lblOp "JP C,label" [| 0xDAuy; 0x00uy; 0x00uy |] (runOf Z80Table.main 0xDA) c (fun _ ->
            [| 0xDAuy; byte (!c &&& 0xFF); byte ((!c >>> 8) &&& 0xFF) |])

    let JP_PO_LBL (c: int ref) =
        lblOp "JP PO,label" [| 0xE2uy; 0x00uy; 0x00uy |] (runOf Z80Table.main 0xE2) c (fun _ ->
            [| 0xE2uy; byte (!c &&& 0xFF); byte ((!c >>> 8) &&& 0xFF) |])

    let JP_PE_LBL (c: int ref) =
        lblOp "JP PE,label" [| 0xEAuy; 0x00uy; 0x00uy |] (runOf Z80Table.main 0xEA) c (fun _ ->
            [| 0xEAuy; byte (!c &&& 0xFF); byte ((!c >>> 8) &&& 0xFF) |])

    let JP_P_LBL (c: int ref) =
        lblOp "JP P,label" [| 0xF2uy; 0x00uy; 0x00uy |] (runOf Z80Table.main 0xF2) c (fun _ ->
            [| 0xF2uy; byte (!c &&& 0xFF); byte ((!c >>> 8) &&& 0xFF) |])

    let JP_M_LBL (c: int ref) =
        lblOp "JP M,label" [| 0xFAuy; 0x00uy; 0x00uy |] (runOf Z80Table.main 0xFA) c (fun _ ->
            [| 0xFAuy; byte (!c &&& 0xFF); byte ((!c >>> 8) &&& 0xFF) |])

    let CALL_LBL (c: int ref) =
        lblOp "CALL label" [| 0xCDuy; 0x00uy; 0x00uy |] (runOf Z80Table.main 0xCD) c (fun _ ->
            [| 0xCDuy; byte (!c &&& 0xFF); byte ((!c >>> 8) &&& 0xFF) |])

    let CALL_NZ_LBL (c: int ref) =
        lblOp "CALL NZ,label" [| 0xC4uy; 0x00uy; 0x00uy |] (runOf Z80Table.main 0xC4) c (fun _ ->
            [| 0xC4uy; byte (!c &&& 0xFF); byte ((!c >>> 8) &&& 0xFF) |])

    let CALL_Z_LBL (c: int ref) =
        lblOp "CALL Z,label" [| 0xCCuy; 0x00uy; 0x00uy |] (runOf Z80Table.main 0xCC) c (fun _ ->
            [| 0xCCuy; byte (!c &&& 0xFF); byte ((!c >>> 8) &&& 0xFF) |])

    let CALL_NC_LBL (c: int ref) =
        lblOp "CALL NC,label" [| 0xD4uy; 0x00uy; 0x00uy |] (runOf Z80Table.main 0xD4) c (fun _ ->
            [| 0xD4uy; byte (!c &&& 0xFF); byte ((!c >>> 8) &&& 0xFF) |])

    let CALL_C_LBL (c: int ref) =
        lblOp "CALL C,label" [| 0xDCuy; 0x00uy; 0x00uy |] (runOf Z80Table.main 0xDC) c (fun _ ->
            [| 0xDCuy; byte (!c &&& 0xFF); byte ((!c >>> 8) &&& 0xFF) |])

    let CALL_PO_LBL (c: int ref) =
        lblOp "CALL PO,label" [| 0xE4uy; 0x00uy; 0x00uy |] (runOf Z80Table.main 0xE4) c (fun _ ->
            [| 0xE4uy; byte (!c &&& 0xFF); byte ((!c >>> 8) &&& 0xFF) |])

    let CALL_PE_LBL (c: int ref) =
        lblOp "CALL PE,label" [| 0xECuy; 0x00uy; 0x00uy |] (runOf Z80Table.main 0xEC) c (fun _ ->
            [| 0xECuy; byte (!c &&& 0xFF); byte ((!c >>> 8) &&& 0xFF) |])

    let CALL_P_LBL (c: int ref) =
        lblOp "CALL P,label" [| 0xF4uy; 0x00uy; 0x00uy |] (runOf Z80Table.main 0xF4) c (fun _ ->
            [| 0xF4uy; byte (!c &&& 0xFF); byte ((!c >>> 8) &&& 0xFF) |])

    let CALL_M_LBL (c: int ref) =
        lblOp "CALL M,label" [| 0xFCuy; 0x00uy; 0x00uy |] (runOf Z80Table.main 0xFC) c (fun _ ->
            [| 0xFCuy; byte (!c &&& 0xFF); byte ((!c >>> 8) &&& 0xFF) |])
        ...
        member this.Refresh(memory: byte[]) =
            for (start, offset, expected) in entries do
                ...
                while matches && i < expected.Length do
                    if memory[(start + i) &&& 0xFFFF] <> expected[i] then
                        matches <- false
    ...
    let makeIndex (program: Z80Op list) : ExecutionIndex = ExecutionIndex(program, 0)

    /// Build an index for a CE image loaded at an explicit 16-bit address.
    let makeIndexAt (baseAddress: int) (program: Z80Op list) : ExecutionIndex = ExecutionIndex(program, baseAddress)
