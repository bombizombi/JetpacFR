...
type ControlMap() as self =
    ...
    let addrAt (y: float) : int =
        ...
        viewStart
        + int (clampF 0.0 (float (viewEnd - viewStart - 1)) (y / h * float (viewEnd - viewStart)))
            ...
            if m.Rows.Length > 0 then
                ...
                while y < m.Rows.Length do
                    ...
                    while y2 < m.Rows.Length do
                        ...
                        if n.Kind = r.Kind && n.Heat = r.Heat then
                            y2 <- y2 + 1
                        else
                            y2 <- m.Rows.Length
                    ...
                    for yy in y .. y2 - 1 do
                        ...
                        if m.Rows[yy].SelfMod then
                            dc.DrawLine(selfModPen, Point(0.0, ry), Point(2.0, ry))

                        if m.Rows[yy].Comment then
                            dc.DrawLine(commentPen, Point(2.0, ry), Point(4.0, ry))
