...
module FlameGraphInternal =
    ...
    let formatTicks (t: int64) : string =
      let v = float t
      if abs v >= 1_000_000.0 then sprintf "%.1fM" (v / 1_000_000.0)
      elif abs v >= 1_000.0 then sprintf "%.1fk" (v / 1_000.0)
      else string t
    ...
    let mnemonicOfBytes (b0: uint8) (b1: uint8) (b2: uint8) (b3: uint8) : string =
      ...
      let rec go (rows: Jetpac2.Core.Z80Decode.Row list) =
        ...
        | r :: rest ->
          ...
          let mutable i = 0
          while ok && i < len do
            if r.Mask[i] <> 0uy && bytes[i] <> r.Prefix[i] then ok <- false
            i <- i + 1
...
type FlameGraph() as self =
  ...
  let gridPen = Pen(SolidColorBrush(Color.FromArgb(0x40uy, 0x80uy, 0x80uy, 0x90uy)), 1.0)
  let framePen = Pen(SolidColorBrush(Color.FromArgb(0x55uy, 0x40uy, 0xC4uy, 0xFFuy)), 1.0)
  ...
  let typeface = Typeface(FontFamily("Consolas"), FontStyles.Normal, FontWeights.Normal, FontStretches.Normal)
  let boldTypeface = Typeface(FontFamily("Consolas"), FontStyles.Normal, FontWeights.Bold, FontStretches.Normal)
  ...
  let laneEdgePen = Pen(SolidColorBrush(Color.FromArgb(0xB0uy, 0xE8uy, 0xE8uy, 0xE8uy)), 2.0)
  ...
  member this.RangeA
    with get () = rangeA
    and set v = rangeA <- v; this.InvalidateVisual()

  member this.RangeB
    with get () = rangeB
    and set v = rangeB <- v; this.InvalidateVisual()
  ...
  member private this.TickAt(x: float) =
    origin + int64 (Math.Round(x / ppTick))

  member private this.XOf(tick: int64) =
    float (tick - origin) * ppTick
  ...
  override this.OnMouseMove(e: MouseEventArgs) =
    ...
    if dragging then
      ...
      if abs dx > 3.0 || abs dy > 3.0 then dragMoved <- true
      if dragMoved then
        match window with
        | Some w ->
  ...
  override this.OnMouseUp(e: MouseButtonEventArgs) =
    ...
    if e.ChangedButton = MouseButton.Left && dragging then
      ...
      | Some w, Some r ->
        ...
        let entryIdx = if r.EndIndex > r.StartIndex then r.EndIndex - 1 else r.StartIndex
  ...
  member private this.RectAt(p: Point) : FlameRect option =
    ...
    | Some w when p.X >= 0.0 && p.X <= this.ActualWidth ->
      ...
      while i < w.Rects.Length && w.Rects[i].StartTick <= tick do
        let r = w.Rects[i]
        if r.EndTick > tick && r.Depth = depth then best <- Some r
        i <- i + 1
      best
    | _ -> None
    ...
    if w <= 0.0 || h <= 0.0 then ()
      ...
      let ft (text: string) (size: float) (fg: Brush) (bold: bool) =
        FormattedText(text, CultureInfo.InvariantCulture, FlowDirection.LeftToRight,
                      (if bold then boldTypeface else typeface), size, fg, dpi.PixelsPerDip)
      dc.DrawRectangle(bgBrush, borderPen, Rect(0.0, 0.0, w, h))
      match window with
      | None ->
        let hint = ft "flame graph: select a range and press Flame A / Flame B (or let auto-build run)" 11.0 labelFg false
      ...
      | Some win ->
        ...
        if playheadFrame >= 0 && win.FirstFrame >= 0 then
          let tick =
            if playheadFrame < win.FirstFrame then 0L
            elif playheadFrame - win.FirstFrame >= win.FrameTicks.Length then win.EndTick
            else win.FrameTicks[playheadFrame - win.FirstFrame]
          let x = this.XOf tick
        ...
        | Some p, Some r ->
          ...
          let lines =
            [ labelFor (int r.Entry) + sprintf " ($%04X)" (int r.Entry)
              ...
              | _ -> "" ]
          let widest = lines |> List.map (fun s -> (ft s 11.0 Brushes.White false).Width) |> List.max
