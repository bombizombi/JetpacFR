...
type ExecutionTraceWriter(path: string) =
  ...
  do
    writer.WriteLine("# tstate|address|opcode|nextpc|writes(address:old:new,...)")
  ...
  member this.Append(events: Jetpac.Core.InstructionTraceEvent list) =
    for event in events do
      let writes =
        event.Writes
        |> List.map (fun write -> sprintf "%04X:%02X:%02X" write.Address (int write.OldValue) (int write.NewValue))
        |> String.concat ","
      pending.Add(sprintf "%d|%04X|%02X|%04X|%s" event.TState event.Address (int event.Opcode) event.NextPc writes)
    if (DateTime.UtcNow - lastFlush).TotalSeconds >= 1.0 then
      this.Flush()

  member this.Flush() =
    for line in pending do writer.WriteLine line
    pending.Clear()
    writer.Flush()
