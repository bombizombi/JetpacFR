...
type Session(romPath: string, tzxPath: string) as self =
    let store =
        CheckpointStore(Path.Combine(Directory.GetCurrentDirectory(), "checkpoints"))
    ...
    member this.RestoreCheckpoint(id: string) =
        ...
        with _ ->
            ...
            for row, bit in oldKeys do
                oracle.SetKey(row, bit, true)
    ...
    member private this.AutoCaptureIfDue() =
        ...
        | Some interval when interval > 0 && frame > 0 && frame % interval = 0 ->
            ...
            self.CaptureCheckpoint(id, Some currentId, sprintf "automatic frame %d" frame)
            |> ignore
        | _ -> ()
    ...
    member this.RunTimeline(framesN: int, timeline: InputTimeline) =
        if framesN < 0 then
            invalidArg (nameof framesN) "Frame count cannot be negative"
        ...
        for _ in 1..framesN do
            ...
            while not (List.isEmpty pending) && (List.head pending).TState < frameEnd do
                ...
                while port.CycleCount() < target do
                    port.Step()
    ...
    member this.SetKey(row: int, bit: int, pressed: bool) =
        ...
        pressedKeys <-
            if pressed then
                Set.add (row, bit) pressedKeys
            else
                Set.remove (row, bit) pressedKeys
