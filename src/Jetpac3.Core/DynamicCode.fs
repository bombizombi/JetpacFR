...
module DynamicDecoder =
  let private byteAt (memory: byte[]) address = int (Array.item (address &&& 0xFFFF) memory)
  ...
  let private signed value = if value >= 0x80 then value - 0x100 else value
  ...
  let private baseLength opcode =
    ...
    | 0xC6 | 0xCE | 0xD6 | 0xDE
    | 0xD3 | 0xDB | 0xE6 | 0xEE | 0xF6 | 0xFE -> 2
    | 0xCB -> 2
    | 0xED -> 2
    | 0xDD | 0xFD -> 2
    | _ -> 1
  ...
  let private successors (memory: byte[]) address opcode length =
    ...
    let absoluteJumps =
      [ 0xC3; 0xC2; 0xC4; 0xCA; 0xCC; 0xD2; 0xD4; 0xDA; 0xDC
        0xE2; 0xE4; 0xEA; 0xEC; 0xF2; 0xF4; 0xFA; 0xFC ]
    let conditionalReturns =
    ...
    elif opcode = 0xCD then [ next; targetFor memory address length ]
    elif opcode = 0xE9 then [] // JP (HL)/(IX)/(IY): concrete target is runtime-only.
    else [ next ]

  let private cycleCount opcode =
    match opcode with
    | 0x00 | 0x76 -> 4
    | 0x3E | 0x06 | 0x0E | 0x16 | 0x1E | 0x26 | 0x2E -> 7
    | 0xC3 | 0xCD -> 10
    | 0xC9 -> 10
    | 0x18 -> 12
    | 0x10 -> 13
  ...
  let decode (memory: byte[]) (address: int) : DecodedInstruction =
    if isNull memory || memory.Length <> 0x10000 then invalidArg (nameof memory) "Decoder requires 65536 bytes"
  ...
  let decode (memory: byte[]) (entry: int) : DecodedBlock =
    let rec loop address instructions dependencies totalCycles steps =
      ...
      if ends instruction || steps >= 255 then
        ...
        loop ((instruction.Address + instruction.Length) &&& 0xFFFF) nextInstructions nextDependencies nextCycles (steps + 1)
...
module DynamicBlocksExecution =
  let execute (machine: Jetpac2.Core.Machine) (block: DecodedBlock) : BlockExecutionResult =
    ...
    for instruction in block.Instructions do
      if machine.Regs.Pc() <> instruction.Address then equivalent <- false
      if equivalent then machine.Step()
    { StartPc = startPc
...
type CodeCache() as self =
  ...
  member this.Invalidate(address: int) =
    ...
    | true, entries ->
      for entry in entries do instructions.Remove entry |> ignore
