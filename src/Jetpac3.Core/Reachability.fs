...
module ConcreteExplorer =
  ...
  let explore (machine: Jetpac2.Core.Machine) (cache: CodeCache) (policy: ExplorationPolicy) (cancel: CancellationToken) : ExplorationGraph =
    ...
    let mutable count = 0
    while failure.IsNone && count < policy.MaxInstructions && machine.CycleCount() - startTState < policy.MaxTStates && not cancel.IsCancellationRequested do
      ...
      for successor in instruction.Successors do
        if successor <> ((pc + instruction.Length) &&& 0xFFFF) || policy.ConfiguredAddresses.Contains successor then
      ...
      count <- count + 1
    let failure =
      if cancel.IsCancellationRequested then Some "exploration cancelled"
      elif failure.IsSome then failure
      elif count >= policy.MaxInstructions then Some "instruction limit reached"
      elif machine.CycleCount() - startTState >= policy.MaxTStates then Some "T-state limit reached"
      else None
