...
module BranchLabel =
  ...
  let create (label: string) = BranchLabel(if isNull label then "" else label)
...
module InputTimeline =
  ...
  let append event timeline = { timeline with Events = timeline.Events @ [ event ] }
module InputTimelineCodec =
  ...
  let parse (text: string) : InputTimeline =
    if isNull text then invalidArg (nameof text) "Timeline text cannot be null"
    let lines =
    ...
    if version <> 1 then failwithf "unsupported input timeline version %d" version
    ...
    let events =
      ...
      |> List.choose (fun line ->
        ...
        | _ -> None)
    { Version = version; RomSha256 = rom; TzxSha256 = tzx; Events = events }
