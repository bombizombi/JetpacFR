...
module CheckpointSchema =
  ...
  let RequiredStateKeys =
    [ "af"; "bc"; "de"; "hl"; "af2"; "bc2"; "de2"; "hl2"
      "ix"; "iy"; "sp"; "pc"; "i"; "r"; "wz"; "iff1"; "iff2"
      "im"; "halted"; "border"; "beeper"; "tapeEar"; "cycles"
      "videoNextTime"; "nextWrap" ]
...
module StateText =
  ...
  let validate (text: string) =
    if isNull text then failwith "checkpoint state.txt is null"
    let kv = parse text
    let missing =
    ...
    let parseHex key =
      try Convert.ToInt32((kv.[key]).Replace("0x", ""), 16) |> ignore
      with ex -> failwithf "checkpoint state key %s is not valid hexadecimal: %s" key ex.Message
    let parseInt64 key =
      try Int64.Parse kv.[key] |> ignore
      with ex -> failwithf "checkpoint state key %s is not a valid integer: %s" key ex.Message
    let parseBool key =
    ...
    [ "af"; "bc"; "de"; "hl"; "af2"; "bc2"; "de2"; "hl2"; "ix"; "iy"
      "sp"; "pc"; "i"; "r"; "wz"; "im"; "border" ]
    |> List.iter parseHex
...
module CanonicalState =
  ...
  let private hardwareText (trace: HardwareTrace) =
    let ports =
      trace.PortEvents
      |> List.map (fun event -> sprintf "%d:%04x:%02x:%d:%b" event.Tick event.Port event.Value event.Border event.Beeper)
    let writes =
      trace.MemoryWrites
      |> List.map (fun event -> sprintf "%d:%04x:%02x:%02x" event.Tick event.Address event.OldValue event.NewValue)
    String.concat "|" (ports @ writes)

  let compute (memory: byte[]) (stateText: string) (trace: HardwareTrace) : CanonicalStateHash =
    if isNull memory || memory.Length <> 0x10000 then invalidArg (nameof memory) "Canonical state requires 65536 bytes"
    ...
    let full = digest (Text.Encoding.UTF8.GetBytes (String.concat "|" (cpu :: hardware :: pages)))
    { Cpu = cpu; Hardware = hardware; MemoryPages = pages; Full = full }

  let checkpoint (checkpoint: Checkpoint) = compute checkpoint.Memory checkpoint.StateText HardwareTrace.empty
...
module private MetaJson =
  ...
  let write (frame: int) (parent: string option) (input: string) (created: DateTime) =
    let parentJs = match parent with Some p -> "\"" + esc p + "\"" | None -> "null"
    sprintf "{\"version\":%d,\"frame\":%d,\"parent\":%s,\"input\":\"%s\",\"created\":\"%s\"}"
      CheckpointSchema.CurrentVersion frame parentJs (esc input) (created.ToString("o"))

  let private readStr (key: string) (json: string) : string option =
    let m = Text.RegularExpressions.Regex.Match(json, "\"" + key + "\"\\s*:\\s*\"((?:[^\"\\\\]|\\\\.)*)\"")
    if m.Success then Some(m.Groups.[1].Value.Replace("\\\"", "\"").Replace("\\\\", "\\")) else None

  let private readInt (key: string) (json: string) : int option =
    let m = Text.RegularExpressions.Regex.Match(json, "\"" + key + "\"\\s*:\\s*(-?\\d+)")
    if m.Success then Some(int m.Groups.[1].Value) else None

  let parse (json: string) : int option * int option * string option * string option * DateTime option =
    let created =
      readStr "created" json
      |> Option.bind (fun s -> match DateTime.TryParse(s) with | true, d -> Some d | _ -> None)
...
type CheckpointStore(rootDir: string) =
  ...
  member private this.ValidateId(id: string) =
    if String.IsNullOrWhiteSpace id then invalidArg (nameof id) "Checkpoint ID cannot be empty"
    if id = "." || id = ".." || Path.GetFileName id <> id || id.IndexOfAny([| Path.DirectorySeparatorChar; Path.AltDirectorySeparatorChar |]) >= 0 then
      invalidArg (nameof id) "Checkpoint ID must be a single safe path component"

  member this.Init() = Directory.CreateDirectory rootDir |> ignore
  ...
  member this.Save(c: Checkpoint) =
    ...
    if c.Memory.Length <> 0x10000 then failwithf "checkpoint %s memory must be exactly 65536 bytes" c.Id
  ...
  member this.Load(id: string) : Checkpoint =
    let dir = this.Dir id
    if not (Directory.Exists dir) then failwithf "checkpoint %s does not exist" id
    ...
    for path in [ memoryPath; statePath; metaPath ] do
      if not (File.Exists path) then failwithf "checkpoint %s is incomplete; missing %s" id path
    let memory = File.ReadAllBytes memoryPath
    if memory.Length <> 0x10000 then failwithf "checkpoint %s memory is %d bytes; expected 65536" id memory.Length
    ...
    | Some v -> failwithf "checkpoint %s uses unsupported schema version %d (expected %d)" id v CheckpointSchema.CurrentVersion
    | None -> failwithf "checkpoint %s metadata has no schema version" id
    let kv = StateText.parse state
    let hex (k: string) = Convert.ToInt32(kv.[k].Replace("0x", ""), 16)
