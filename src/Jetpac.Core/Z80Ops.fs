...
module Z80Ops =
  ...
  let private blockLoad (z80: Z80) (increment: bool) (repeat: bool) =
    ...
    let preservedFlags = z80.Flags() &&& (Flags.Sign() ||| Flags.Zero() ||| Flags.Carry())
    let flagsFromBits =
  ...
  let private blockCompare (z80: Z80) (increment: bool) (repeat: bool) =
    ...
    let preservedFlags =
      z80.Flags() &&& ~~~(Flags.Flag3() ||| Flags.Flag5() ||| fromSubtractMask ||| Flags.Overflow())
    let flagsFromBits =
      (if flagBits &&& 0x08 <> 0 then Flags.Flag3() else Flags())
      ||| (if flagBits &&& 0x02 <> 0 then Flags.Flag5() else Flags())
    let flagsFromBc = if newBc <> 0 then Flags.Overflow() else Flags()
    z80.SetFlags(preservedFlags ||| flagsFromBits ||| flagsFromBc ||| (fromSubtractMask &&& subtractFlags))
  ...
  let private edOp (z80: Z80) (opcode: int) =
    ...
    | 1 ->
      ...
      | 2 ->
        ...
        let struct (result, flags) =
          if q = 0 then Alu.sbc16 lhs rhs (z80.Flags().carry)
          else Alu.adc16 lhs rhs (z80.Flags().carry)
    ...
    | 0 ->
      ...
      | 1 ->
        if q = 0 then
          ...
          let struct (result, flags) = Alu.add16 (z80.Regs.Get(hlHighLow ctx)) rhs (z80.Flags())
