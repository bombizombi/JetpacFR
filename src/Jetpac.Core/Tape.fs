...
module Tape =
  ...
  let parseTzx (data: byte[]) : TzxBlock list =
    ...
    let mutable pos = 7
    if data.[pos] <> 0x1Auy then failwith "Malformed TZX header (missing 0x1A)."
    pos <- pos + 1
    if data.[pos] = 0x1Auy then pos <- pos + 1 // tolerate single or double 0x1A terminator
    pos <- pos + 2 // version
    ...
    while pos < data.Length do
      ...
      | 0x11uy -> // turbo: pause(2) pilot(2) sync1(2) sync2(2) bit0(2) bit1(2) pause(2) usedBits(3) len(2) data
        ...
        blocks.Add(TurboData(u16 data pos, u16 data (pos + 2), u16 data (pos + 8), u16 data (pos + 10), data.[pos + 19 .. pos + 19 + len - 1]))
        pos <- pos + 19 + len
      | 0x12uy -> pos <- pos + 4; blocks.Add(Skipped id)
      | 0x13uy ->
        let n = int data.[pos]
        pos <- pos + 1 + n * 2
      ...
      | 0x14uy ->
        ...
        blocks.Add(Skipped id)
      | 0x15uy ->
        let len = int data.[pos + 6] ||| (int data.[pos + 7] <<< 8) ||| (int data.[pos + 8] <<< 16)
        pos <- pos + 9 + len
        blocks.Add(Skipped id)
      | 0x16uy -> pos <- pos + 3; blocks.Add(Skipped id)
      | 0x17uy ->
      ...
      | 0x19uy -> // generalized data: symbol-coded; skip whole
        ...
        pos <- p
        blocks.Add(Skipped id)
      | 0x20uy -> pos <- pos + 2; blocks.Add(Skipped id)
      | 0x21uy | 0x28uy | 0x30uy ->
        let n = int data.[pos]
        pos <- pos + 1 + n
        blocks.Add(Skipped id)
      | 0x22uy | 0x25uy -> blocks.Add(Skipped id)
      | 0x23uy | 0x24uy -> pos <- pos + 2; blocks.Add(Skipped id)
      | 0x26uy -> pos <- pos + 4; blocks.Add(Skipped id)
      | 0x27uy -> pos <- pos + 1; blocks.Add(Skipped id)
      | 0x31uy ->
      ...
      | 0x34uy ->
        ...
        pos <- pos + 3 + descLen + n
        blocks.Add(Skipped id)
      | 0x35uy -> pos <- pos + 4; blocks.Add(Skipped id)
  ...
  let stripFrame (frame: byte[]) : byte[] =
    ...
    if n >= 2 && (frame.[0] = 0xFFuy || frame.[0] = 0x00uy) then
      if n >= 3 then
        ...
        for i in 0 .. n - 2 do
          x <- x ^^^ frame.[i]
        if x = frame.[n - 1] then frame.[1 .. n - 2] // flag + payload + checksum
        else frame.[1 ..] // flag + payload only
      else
        frame.[1 ..]
    else
      frame
  ...
  let framesFor (block: TzxBlock) : FrameBlock list =
    ...
    | StandardData (pauseMs, frame) ->
      [ { Data = frame
          ...
          PilotEdges = if frame.[0] &&& 0x80uy <> 0uy then PilotDataEdges else PilotHeaderEdges } ]
...
type Tape() =
  ...
  member private this.CurrentBlock() : Tape.FrameBlock option =
    if currentBlockIndex < blocks.Length then Some blocks.[currentBlockIndex] else None
  ...
  member private this.NextBit() : Tape.State =
    ...
    | Some block ->
      ...
      if byteOffset >= block.Data.Length then
        ...
        bitCycles <-
          if block.Data.[byteOffset] &&& (1uy <<< (7 - bitOffset % 8)) <> 0uy then block.Bit1Cycles
          else block.Bit0Cycles
