...
/// ULA video constants (peripherals/Video.cpp).
...
type Video(memory: Memory) =
    ...
    let mutable nextLineCycles = 0UL
    ...
    let screenBuffer =
        Array.zeroCreate<byte> (VideoConstants.VisibleWidth * VideoConstants.VisibleHeight * 4)

    member this.SetBorder(v: int) = border <- v &&& 7
    member this.SetPage(v: int) = page <- v

    /// Reconstruct the scanline/flash phase (used by Spectrum48.LoadState).
    member this.SetState(scanline: int, flash: int, flashState: bool) =
        currentLine <- scanline
        flashCounter <- flash
        flashOn <- flashState

    member this.Poll(numCycles: uint64) : bool =
        let mutable irq = false
        totalCycles <- totalCycles + numCycles

        while totalCycles > nextLineCycles do
            if this.NextScanLine() then
                irq <- true

            nextLineCycles <- nextLineCycles + uint64 VideoConstants.CyclesPerScanLine
        ...
        if displayLine < VideoConstants.VSyncLines then
            ...
            if y >= VideoConstants.ScreenHeight || y < 0 then
                ...
                for x in 0 .. VideoConstants.ColumnCount - 1 do
                    line.Pixels[x] <- memory.RawRead(page, screenOffset + x)

                    line.Attrs[x] <-
                        memory.RawRead(
                            page,
                            VideoConstants.AttributeDataOffset + charRow * VideoConstants.ColumnCount + x
                        )
    ...
    member this.BlitTo() : byte[] =
        ...
        for y in 0 .. VideoConstants.VisibleHeight - 1 do
            ...
            if
                y < VideoConstants.YBorder
                || y >= VideoConstants.YBorder + VideoConstants.ScreenHeight
            then
                ...
                for x in 0 .. VideoConstants.ColumnCount - 1 do
                    ...
                    // Note: the C++ names are swapped; the net effect is that a set bit
                    // draws the INK colour.
                    let paperColor = if invert then pal[index1] else pal[index2]
                    let penColor = if invert then pal[index2] else pal[index1]

                    let mutable i =
                        (y * VideoConstants.VisibleWidth + VideoConstants.XBorder + x * 8) * 4

                    for bit in 0..7 do
                        let color =
                            if pixel &&& (1 <<< (7 - bit)) <> 0 then
                                paperColor
                            else
                                penColor
