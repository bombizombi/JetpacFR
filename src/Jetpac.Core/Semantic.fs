...
module Semantic =
    ...
    let jetmanVelIncY (m: Machine) =
        ...
        | 0x755C ->
            ...
            let struct (result, flags) =
                Alu.sbc16 (m.Regs.Get R16.HL) (m.Regs.Get R16.DE) (m.Flags().carry)
    ...
    let finalFrags (m: Machine) =
        ...
        | 0x69B4 ->
            ...
            let struct (result, flags) =
                Alu.sbc16 (m.Regs.Get R16.HL) (m.Regs.Get R16.BC) (m.Flags().carry)
    ...
    let hook (addr: int) : (Machine -> unit) option =
        ...
        then
            Some finalFrags
        elif addr >= 0x70C3 && addr <= 0x70C5 then
            Some fragBits2
        elif addr >= 0x71F4 && addr <= 0x71F6 then
            Some fragBits2
        elif addr >= 0x73B5 && addr <= 0x73B7 then
            Some fragBits2
        elif
            addr = 0x60FB
            || addr = 0x60FC
        ...
        then
            Some fragBits2
        elif addr >= 0x691C && addr <= 0x691E then
            Some fragBits
        elif addr >= 0x692D && addr <= 0x6936 then
            Some fragBits
        elif addr >= 0x69D2 && addr <= 0x69D4 then
            Some fragBits
        elif addr >= 0x6A04 && addr <= 0x6A09 then
            Some fragBits
        elif addr >= 0x6A15 && addr <= 0x6A17 then
            Some fragBits
        elif
            addr = 0x6DFE
            || addr = 0x6DFF
        ...
        then
            Some fragBits
        elif addr >= 0x7624 && addr <= 0x7627 then
            Some fragBits
        elif addr >= 0x7631 && addr <= 0x7634 then
            Some fragBits
        elif addr >= 0x63BD && addr <= 0x63C5 then
            Some stringCompare
        elif
            addr = 0x65EF
            || addr = 0x65F0
        ...
        then
            Some deScan
        elif addr >= 0x6742 && addr <= 0x6747 then
            Some colourShift
        elif addr >= 0x6A2A && addr <= 0x6A30 then
            Some colourShift
        elif addr >= 0x6143 && addr <= 0x614A then
            Some zeroLoop
        elif addr >= 0x6921 && addr <= 0x6926 then
            Some zeroLoop
        elif addr >= 0x6E56 && addr <= 0x6E5F then
            Some actorProbe
        elif addr >= 0x7414 && addr <= 0x7417 then
            Some scale8
        elif
            addr = 0x60CD
            || addr = 0x60CE
        ...
        then
            Some smallBits
        elif addr >= 0x6A37 && addr <= 0x6A45 then
            Some actorColour
        elif addr >= 0x6E27 && addr <= 0x6E38 then
            Some collisionCheck
        elif addr >= 0x71DD && addr <= 0x71F3 then
            Some attrCoord
        elif addr >= 0x6E9D && addr <= 0x6EAE then
            Some rocketShift
        elif addr >= 0x6EAF && addr <= 0x6EB9 then
            Some rocketRotate
        elif addr >= 0x6EE2 && addr <= 0x6EEF then
            Some actorDataCopy
        elif addr >= 0x683D && addr <= 0x6844 then
            Some sfxCopyTwo
        elif addr >= 0x684A && addr <= 0x6864 then
            Some sfxTail
        elif addr >= 0x6235 && addr <= 0x6252 then
            Some menuLoop
        elif addr >= 0x6254 && addr <= 0x630E then
            Some menuCursor
        elif addr >= 0x61BF && addr <= 0x61E5 then
            Some startGame
        elif addr >= 0x6174 && addr <= 0x6188 then
            Some playerInit
        elif addr >= 0x619C && addr <= 0x61BC then
            Some resetPlayerState
        elif addr = 0x6334 then
            Some performJump
        elif addr >= 0x6523 && addr <= 0x6534 then
            Some collectRocketItem
        elif addr >= 0x7425 && addr <= 0x755B then
            Some jetmanVelIncY
        elif addr >= 0x7134 && addr <= 0x714B then
            Some displayString
        elif addr >= 0x714C && addr <= 0x716B then
            Some writeAsciiChars
        elif addr >= 0x716C && addr <= 0x7191 then
            Some printLabel
        elif addr >= 0x7192 && addr <= 0x71AC then
            Some labelSetup
        elif addr >= 0x7308 && addr <= 0x7326 then
            Some readInputLR
        elif addr >= 0x7327 && addr <= 0x7339 then
            Some copyJoystickState
        elif addr >= 0x733A && addr <= 0x733C then
            Some readInterface2
        elif addr >= 0x6344 && addr <= 0x635E then
            Some mainFlow
        elif addr >= 0x6361 && addr <= 0x636F then
            Some updateHiScore
        elif addr >= 0x6396 && addr <= 0x63A4 then
            Some scoreCompare
        elif addr >= 0x63A6 && addr <= 0x63B7 then
            Some hiScoreUpdate
        elif addr >= 0x64E3 && addr <= 0x64F0 then
            Some itemUpdate
        elif addr >= 0x665E && addr <= 0x6666 then
            Some moduleReset
        elif addr >= 0x6963 && addr <= 0x6983 then
            Some frameWork
        elif addr >= 0x6987 && addr <= 0x6998 then
            Some dispatchRestore
        elif addr >= 0x6999 && addr <= 0x69A3 then
            Some actorUpdateTail
        elif addr >= 0x60DF && addr <= 0x60F6 then
            Some rocketSetup
        elif addr >= 0x653D && addr <= 0x65A7 then
            Some jetmanInteract
        elif addr >= 0x65A8 && addr <= 0x65DB then
            Some jetmanCollect
        elif addr >= 0x6EF5 && addr <= 0x6F21 then
            Some rocketUpdate
        elif addr >= 0x6F3E && addr <= 0x6F8C then
            Some rocketWriter
        elif addr >= 0x685D && addr <= 0x686D then
            Some actorClick
        elif addr >= 0x6873 && addr <= 0x6891 then
            Some actorInit
        elif addr >= 0x712C && addr <= 0x714B then
            Some scoreDisplay
        elif addr >= 0x720E && addr <= 0x7223 then
            Some coordToAttr
        elif addr >= 0x60CF && addr < 0x60DF then
            Some copyRocketData
        elif addr >= 0x60A9 && addr < 0x60CF then
            Some frameStarter
        elif addr >= 0x608A && addr <= 0x60A6 then
            Some mainLoop
        elif addr = 0x67FB || addr = 0x6801 then
            Some sfxExplosionParams
        elif addr >= 0x6807 && addr <= 0x680F then
            Some sfxExplosionHi
        elif addr >= 0x6810 && addr <= 0x681B then
            Some explosionLoop
        elif addr >= 0x681C && addr <= 0x6831 then
            Some sfxPlayExplosion
        elif addr = 0x71B8 || addr = 0x71C6 then
            Some screenClearEntry
        elif addr >= 0x71BF && addr <= 0x71C5 then
            Some screenClearLoop
        else
            None
