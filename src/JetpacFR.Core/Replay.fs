...
module ReplayStore =
  ...
  let private sha256File (path: string) =
    ...
    else sha256Bytes (File.ReadAllBytes path)
  ...
  let path (game: GameManifest) = Path.Combine(game.GameDirectory, FileName)

  let private quote (value: string) = JsonSerializer.Serialize(if isNull value then "" else value)
    ...
    let eventText =
      ...
      |> List.map (fun e ->
        sprintf
          ...
          e.Frame e.Row e.Bit e.Pressed)
      |> String.concat ","
    sprintf
      "{\"format\":%s,\"version\":%d,\"gameId\":%s,\"assetFingerprint\":{\"romSha256\":%s,\"tzxSha256\":%s,\"programSha256\":%s,\"programAddress\":%s},\"events\":[%s]}"
      (quote Format) Version (quote game.GameId)
      (quote fp.RomSha256) (quote fp.TzxSha256) (quote fp.ProgramSha256)
      (optionalInt fp.ProgramAddress) eventText
  ...
  let private parseEvent (value: JsonElement) =
    ...
    | Some frame, Some row, Some bit, (true, pressed)
      when frame >= 0 && row >= 0 && row < 8 && bit >= 0 && bit < 5
           && (pressed.ValueKind = JsonValueKind.True || pressed.ValueKind = JsonValueKind.False) ->
      Some { Frame = frame; Row = row; Bit = bit; Pressed = pressed.GetBoolean() }
    | _ -> None

  let private parseEvents (array: JsonElement) =
    if array.ValueKind <> JsonValueKind.Array then None
    else
      let parsed = array.EnumerateArray() |> Seq.map parseEvent |> Seq.toList
      if parsed |> List.forall Option.isSome then Some(parsed |> List.choose id) else None
  ...
  let tryLoad (game: GameManifest) : ReplayLoad =
    let target = path game
    if not (File.Exists target) then ReplayMissing
    else
      try
        ...
        if root.ValueKind = JsonValueKind.Array then
          ...
          if format <> Format then ReplayIgnored "unsupported replay format"
          elif version <> Version then ReplayIgnored(sprintf "unsupported replay version %d" version)
          elif gameId <> game.GameId then ReplayIgnored "replay belongs to another game"
          elif not (matchesFingerprint game root) then ReplayIgnored "replay assets do not match the selected game"
          else
      ...
      with ex -> ReplayIgnored ex.Message
