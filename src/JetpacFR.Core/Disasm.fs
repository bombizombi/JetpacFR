...
module Disasm =
    ...
    type Insn = { Text: string; Length: int }
        ...
        if idx < 0 then
            ...
            let operands =
                ...
                |> Array.map (fun o ->
                    ...
                    | "(HL)" ->
                        if withDisp then
                            sprintf "(%s%s0x%02X)" ixName (sign d) (abs d)
                        else
                            sprintf "(%s)" ixName
                    | "HL" -> ixName
                    | "H" -> ixName + "H"
    ...
    let private renderBase (get: int -> byte) (pc: int) (ix: (string * int * bool) option) : string =
        ...
        let text =
            ...
            | 0x11
            | 0x21
            | 0x31 -> sprintf "LD %s,0x%04X" reg16[rp] (nn ())
            | 0x02 -> "LD (BC),A"
            | 0x0A -> "LD A,(BC)"
            | 0x12 -> "LD (DE),A"
            | 0x1A -> "LD A,(DE)"
            ...
            | 0x13
            | 0x23
            | 0x33 -> sprintf "INC %s" reg16[rp]
            | 0x0B
            | 0x1B
            | 0x2B
            | 0x3B -> sprintf "DEC %s" reg16[rp]
            | 0x09
            | 0x19
            | 0x29
            | 0x39 -> sprintf "ADD HL,%s" reg16[rp]
            | 0x04
            | 0x0C
            | 0x14
            | 0x1C
            ...
            | 0x2C
            | 0x34
            | 0x3C -> sprintf "INC %s" reg8[(op >>> 3) &&& 7]
            | 0x05
            | 0x0D
            | 0x15
            | 0x1D
            ...
            | 0x2D
            | 0x35
            | 0x3D -> sprintf "DEC %s" reg8[(op >>> 3) &&& 7]
            | 0x06
            | 0x0E
            | 0x16
            | 0x1E
            | 0x26
            | 0x2E
            | 0x3E -> sprintf "LD %s,0x%02X" reg8[(op >>> 3) &&& 7] (n1 ())
            | 0x07 -> "RLCA"
            | 0x0F -> "RRCA"
            | 0x17 -> "RLA"
            | 0x1F -> "RRA"
            ...
            | 0x28
            | 0x30
            | 0x38 -> sprintf "JR %s,0x%04X" conds[(op >>> 3) &&& 3] (target ())
            | 0x27 -> "DAA"
            | 0x2F -> "CPL"
            | 0x37 -> "SCF"
            | 0x3F -> "CCF"
            ...
            | _ when op >= 0x40 && op <= 0x7F ->
                if op = 0x76 then
                    ...
                    sprintf "LD %s,%s" reg8[(op >>> 3) &&& 7] reg8[op &&& 7]
            ...
            | 0xE8
            | 0xF0
            | 0xF8 -> sprintf "RET %s" conds[cc]
            | 0xC1
            | 0xD1
            | 0xE1
            | 0xF1 -> sprintf "POP %s" popPush[(op >>> 4) &&& 3]
            | 0xC5
            | 0xD5
            | 0xE5
            | 0xF5 -> sprintf "PUSH %s" popPush[(op >>> 4) &&& 3]
            | 0xC2
            | 0xCA
            | 0xD2
            | 0xDA
            ...
            | 0xEA
            | 0xF2
            | 0xFA -> sprintf "JP %s,0x%04X" conds[cc] (nn ())
            ...
            | 0xEC
            | 0xF4
            | 0xFC -> sprintf "CALL %s,0x%04X" conds[cc] (nn ())
            ...
            | 0xEF
            | 0xF7
            | 0xFF -> sprintf "RST 0x%02X" (op &&& 0x38)
            | 0xC9 -> "RET"
    ...
    let private cbText (op: int) (opnd: string) : string =
        if op < 0x40 then
            sprintf "%s %s" rot[(op >>> 3) &&& 7] opnd
        elif op < 0x80 then
            sprintf "BIT %d,%s" ((op >>> 3) &&& 7) opnd
        elif op < 0xC0 then
            sprintf "RES %d,%s" ((op >>> 3) &&& 7) opnd
        else
            sprintf "SET %d,%s" ((op >>> 3) &&& 7) opnd
    ...
    let private edText (get: int -> byte) (pc: int) : string =
        ...
        | op when op >= 0x40 && op <= 0x7F && (op &&& 7 = 0 || op &&& 7 = 1) ->
            if op = 0x70 then
                ...
                sprintf "IN %s,(C)" reg8[(op >>> 3) &&& 7]
            else
                sprintf "OUT (C),%s" reg8[(op >>> 3) &&& 7]
        | 0x42
        | 0x52
        | 0x62
        | 0x72 -> sprintf "SBC HL,%s" reg16[rp]
        | 0x4A
        | 0x5A
        | 0x6A
        | 0x7A -> sprintf "ADC HL,%s" reg16[rp]
        | 0x43
        | 0x53
        | 0x63
        | 0x73 -> sprintf "LD (0x%04X),%s" (int (at 1) ||| (int (at 2) <<< 8)) reg16[rp]
        | 0x4B
        | 0x5B
        | 0x6B
        | 0x7B -> sprintf "LD %s,(0x%04X)" reg16[rp] (int (at 1) ||| (int (at 2) <<< 8))
        | 0x44
        | 0x4C
        | 0x54
        | 0x5C
    ...
    let decode (get: int -> byte) (pc: int) : Insn =
        ...
        | 0xFD ->
            ...
            if inner = 0xCB then
                ...
                let text =
                    cbText (int (get (pc + 3))) (sprintf "(%s%s0x%02X)" name (sign d) (abs d))
