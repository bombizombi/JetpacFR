    ...
    let tryLoad (path: string) (fp: TimelineFingerprint) : TimelineLoad =
        if not (File.Exists path) then
            ...
            try
                ...
                if r.ReadBytes 4 <> magic then
                    TimelineIgnored "not a Jetpac timeline file"
                else
                    let version = r.ReadUInt16()

                    if version <> Version then
                        TimelineIgnored(sprintf "unsupported timeline version %d" version)
                    else
                        let flags = r.ReadUInt16()

                        if flags &&& FlagsCompressed <> 0us then
                            TimelineIgnored "compressed timelines are not supported yet"
                        else
                            ...
                            if storedGameId <> fp.GameId then
                                TimelineIgnored "timeline belongs to another game"
                            elif
                                storedRom <> fp.RomSha256
                                || storedTzx <> fp.TzxSha256
                                || storedProgram <> fp.ProgramSha256
                            then
                                ...
                                if programAddress <> fp.ProgramAddress then
                                    ...
                                    if not valid then
                                        TimelineIgnored "timeline contains an invalid key event"
                                    else
                                        let startFrame = r.ReadInt32()

                                        if startFrame < 0 then
                                            TimelineIgnored "timeline has an invalid start frame"
                                        else
