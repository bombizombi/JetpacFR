...
module CtrlMapModel =
  ...
  let render
    ...
    for y in 0 .. rowCount - 1 do
      ...
      let kinds =
        samples
        |> Array.map (fun off -> classify blocks (min 0xFFFF (lo + off)))
      let distinct = kinds |> Array.distinct
      let kind =
        if distinct.Length = 1 then distinct[0]
      ...
      rows[y] <- { Kind = kind; Heat = heat; SelfMod = selfModHit; Comment = hasComment }
      ...
      | Some starts, Some memory when ppb >= 8.0 ->
        ...
        while a < viewEnd && guard < 256 do
          ...
          if a < viewEnd && a < 0x10000 then
            ...
            let hex = seq { for i in 0 .. len - 1 -> sprintf "%02X" memory[(a + i) &&& 0xFFFF] } |> String.concat " "
            let cmt = comments |> List.tryFind (fun m -> m.Kind = CommentKind.Line && m.Addr = a) |> Option.map (fun m -> m.Text)
            snips <- { Addr = a; Bytes = hex; Text = insn.Text; Comment = cmt } :: snips
            a <- a + len
