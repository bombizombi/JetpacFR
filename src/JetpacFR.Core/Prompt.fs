...
module Prompt =
  ...
  let private regLine (s: RegSnapshot) =
    sprintf "AF=%04X BC=%04X DE=%04X HL=%04X IX=%04X IY=%04X SP=%04X I=%02X R=%02X"
      s.Af s.Bc s.De s.Hl s.Ix s.Iy s.Sp s.I s.R
  ...
  let generate (c: Contract.RoutineContract) : string =
    ...
    for insn in c.Disassembly do
      append
        (sprintf "%04X  %-28s  x%-7d  ~%d tstates"
          insn.Address insn.Text insn.Executions insn.AvgCycles)
    append ""
    append "## Behavioural contract (observed, not guessed)"
    append (sprintf "- call count: %d" c.CallCount)
    append
      (sprintf "- inclusive T-states: %d (avg %d per call)"
        c.InclusiveTStates
    ...
    append
      (sprintf "- resume addresses (PC after each call site): %s"
        (if List.isEmpty c.ResumeAddresses then "-"
         else c.ResumeAddresses |> List.map (sprintf "%04X") |> String.concat ", "))
    append ""
