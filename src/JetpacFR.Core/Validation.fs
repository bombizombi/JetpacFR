...
module Validation =
    ...
    let private portRegs (r: Jetpac2.Core.RegisterFile) : string =
        sprintf
            ...
            (r.Get Jetpac2.Core.R16.AF)
            (r.Get Jetpac2.Core.R16.BC)
            (r.Get Jetpac2.Core.R16.DE)
            (r.Get Jetpac2.Core.R16.HL)
            (r.Ix())
            (r.Iy())
            (r.Sp())
    ...
    let private oracleRegs (r: Jetpac.Core.RegisterFile) : string =
        sprintf
            "AF=%04X BC=%04X DE=%04X HL=%04X IX=%04X IY=%04X SP=%04X"
            (r.Get Jetpac.Core.R16.AF)
            (r.Get Jetpac.Core.R16.BC)
            (r.Get Jetpac.Core.R16.DE)
            (r.Get Jetpac.Core.R16.HL)
            (r.Ix())
            (r.Iy())
            (r.Sp())
    ...
    let private replayFrame
        ...
        while found.IsNone && port.CycleCount() < frameEnd && steps < 100_000 do
            ...
            if port.Regs.Pc() <> z80.Regs.Pc() || port.CycleCount() <> int64 (z80.CycleCount()) then
                found <-
                    Some
                        { Frame = frame
                          Kind = "instruction"
