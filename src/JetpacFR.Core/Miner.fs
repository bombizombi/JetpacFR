...
module Miner =
    ...
    type CallEdge =
        { Caller: int; Callee: int; Count: int }
        ...
        while i < entries.Length do
            ...
            if e.Length = 0uy then
                ...
                if isCallOp b0 && e.Taken = 1uy then
                    ...
                    if a.CallSites.Count < 64 then
                        a.CallSites.Add(int e.Pc, returnPc)
                    ...
                    a.Loops[key] <-
                        (match a.Loops.TryGetValue key with
                         | true, v -> v
                         | _ -> 0)
                        + 1
            ...
            |> Seq.map (fun a ->
                ...
                let selfMod =
                    ...
                    while addr <= a.SpanHi && not found do
                        if trace.SelfModified[addr &&& 0xFFFF] then
                            found <- true
                ...
                let overlapping =
                    ...
                    |> Seq.exists (fun other ->
                        other.Entry <> a.Entry && other.Entry >= a.SpanLo && other.Entry <= a.SpanHi)
