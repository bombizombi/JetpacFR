...
module GameProject =
  ...
  let parseAddr (s: string) : int =
    ...
    elif t.StartsWith("0x", StringComparison.OrdinalIgnoreCase) then Convert.ToInt32(t.Substring(2), 16)
    else int32 t
  ...
  let loadControl (dir: string) : GameControl =
    ...
    let c =
      { ImageFile = str "image" "original.bin"
        ...
        Blocks = root.TryGetProperty "blocks" |> function true, e -> parseBlocks e | _ -> [] }
    if c.EndExcl <= c.Start then failf "control.json: endExcl (%d) must be > start (%d)" c.EndExcl c.Start
    if c.EndExcl > 0x10000 then failf "control.json: endExcl %d exceeds 64K" c.EndExcl
    ...
    for b in sorted do
      if b.Start < c.Start || b.EndExcl > c.EndExcl || b.Start >= b.EndExcl then
        failf "control.json: block %s [%04X,%04X) outside span [%04X,%04X)" b.Name b.Start b.EndExcl c.Start c.EndExcl
  ...
  let activeVersionPath (dir: string) (c: GameControl) : string option =
    ...
    if not (Directory.Exists vd) then None
    else
      let files = Directory.GetFiles(vd, "v*.fs") |> Array.sort
      if files.Length = 0 then None
      elif c.ActiveVersion >= 0 then
        match files |> Array.tryFind (fun f -> Path.GetFileName(f).StartsWith(sprintf "v%03d_" c.ActiveVersion)) with
        | Some f -> Some f
        | None -> failf "control.json: activeVersion %d has no versions/v%03d_*.fs" c.ActiveVersion c.ActiveVersion
      else Some files[0]
  ...
  let emitImageModule (moduleName: string) (body: string) : string =
    ...
    sb.AppendLine(sprintf "/// GENERATED by --gen-game: the active CE version of this game.") |> ignore
    sb.AppendLine("/// Regenerate via tools/watchgame.ps1 or the game fsproj build target.") |> ignore
  ...
  let emitGameModule (moduleName: string) (imageModuleName: string) (entryPc: int) : string =
    let sb = Text.StringBuilder()
    sb.AppendLine(sprintf "/// GENERATED by --gen-game: drives the CE program on a port Machine.") |> ignore
    sb.AppendLine("/// Fable-clean: no I/O, no GUI - shells and tests share it.") |> ignore
    sb.AppendLine(sprintf "module %s" moduleName) |> ignore
    sb.AppendLine() |> ignore
    sb.AppendLine("open Jetpac2.Core") |> ignore
    sb.AppendLine() |> ignore
    sb.AppendLine(sprintf "/// Entry state: PC=0x%04X, SP=0xFFFE, interrupts off." entryPc) |> ignore
    sb.AppendLine("let entryState () : string =") |> ignore
    sb.AppendLine("  \"af=0000\\nbc=0000\\nde=0000\\nhl=0000\\naf2=0000\\nbc2=0000\\nde2=0000\\nhl2=0000\\nix=0000\\niy=0000\\nsp=FFFE\\n"
