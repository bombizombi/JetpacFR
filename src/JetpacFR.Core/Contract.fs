...
module Contract =
    ...
    let private apiHint (b0: byte) : string =
        ...
        | 0xDD
        | 0xFD -> "Regs.Get/Set (IX/IY, displacement via ReadImm)"
        | 0xD3
        | 0xDB -> "Out / In (port I/O)"
        | op when op >= 0xC0 && op <= 0xFF ->
            let low = op &&& 0x0F

            if op = 0xC9 || low = 0 || low = 8 then
                "Pop16 + Regs.SetPc (return)"
            elif low = 4 || low = 0xC then
                "Push16 (return addr) + Regs.SetPc (call)"
            elif op = 0xC3 || low = 2 || low = 0xA then
                "Regs.SetPc (jump)"
            elif low = 1 then
                "Pop16"
            elif low = 5 then
                "Push16"
            elif low = 6 || low = 0xE then
                "Alu.* (immediate) + SetFlags"
            elif low = 7 || low = 0xF then
                "Push16 (RST return addr) + Regs.SetPc"
            else
                "Regs.SetPc / Push16 / Pop16"
        | op when op >= 0x80 && op <= 0xBF -> "Alu.add8/sub8/cmp8/and8/or8/xor8 + SetFlags"
        | op when op >= 0x40 && op <= 0x7F ->
            if op = 0x76 then
                "HALT (PassTime)"
            elif ((op >>> 3) &&& 7) = 6 || (op &&& 7) = 6 then
                "Read/Write (memory via (HL))"
            else
                "Regs.Get/Set (R8 pairs)"
        | 0x34
        | 0x35 -> "Alu.inc8/dec8 (memory via (HL))"
        | 0x36 -> "Write (LD (HL),n)"
        | 0x02
        | 0x0A
        | 0x12
        ...
        | 0x2A
        | 0x32
        | 0x3A -> "Read/Write (memory via (nn))"
        | 0x10
        | 0x18
        | 0x20
        | 0x28
        | 0x30
        | 0x38 -> "Branch (relative)"
    ...
    let extract (memory: byte[]) (trace: Trace) (r: Miner.Routine) (pcCycles: int64[]) : RoutineContract =
        ...
        while addr <= r.SpanHi && disasm.Count < 512 do
            ...
            let executions =
                if addr < trace.PerPcCount.Length then
                    trace.PerPcCount[addr]
                else
                    0
