"""One-shot refactor: split GUI definitions out of the desktop App.fs.

- CheatEngineWindow.fs  <- CheatEngineWindow type (moved verbatim)
- LauncherView.fs       <- GameImages + Projects modules (moved verbatim)
                          + new LauncherView view module
The game-only view becomes GameOnlyView.fs (new control, written separately)
and is no longer a second window: it is a phase of the main window.
"""
import io
...
gi = find("/// Per-game CE image registration")
dr = find("/// One row of the disassembly lists")
cheat = find("/// Cheat Engine: a secondary window")
jg = find('/// "Just the Game"')
mw = find("type MainWindow")

gi_modules = lines[gi:dr]              # GameImages + Projects
cheat_type = trim_end(lines[cheat:jg])  # CheatEngineWindow type
justgame = lines[jg:mw]                # JustGameWindow (discarded)

header = "namespace JetpacFR.Desktop\n\nopen System\nopen System.IO\n"

# 1) CheatEngineWindow.fs (verbatim move)
with io.open("src/JetpacFR.Desktop/CheatEngineWindow.fs", "w", encoding="utf-8", newline="") as f:
    f.write(header)
    f.write("open System.Windows\nopen System.Windows.Controls\nopen System.Windows.Controls.Primitives\n")
    f.write("open System.Windows.Media\nopen System.Windows.Media.Imaging\nopen System.Windows.Threading\n\n")
    f.write("\n".join(cheat_type).rstrip() + "\n")

# 2) LauncherView.fs (moved modules + new view module)
launcher_module = '''
...
module LauncherView =
    ...
    let build (ctx: LauncherContext) : DockPanel * (unit -> unit) =
        ...
        let buildRow (g: GameManifest) =
            ...
            chip
                status
                (sprintf "control file: %s" (if FileInfo(Path.Combine(g.GameDirectory, "control.json")).Exists then "yes" else "no"))
                (FileInfo(Path.Combine(g.GameDirectory, "control.json")).Exists)

            chip
                status
                (sprintf "CE program: %s" (if GameRegistry.tryFind g.GameId |> Option.isSome then "yes" else "no"))
        ...
        header.Children.Add(
            TextBlock(Text = "ZX Spectrum Game Changer", Foreground = normal, FontSize = 26.0, FontWeight = FontWeights.Bold)
        )
        |> ignore

        header.Children.Add(
            TextBlock(
                Text = "Pick a project, then choose how to open it: the emulator, the cheat engine, or the bare game view.",
                Foreground = dim,
                FontSize = 13.0,
...
with io.open("src/JetpacFR.Desktop/LauncherView.fs", "w", encoding="utf-8", newline="") as f:
    f.write(header.rstrip() + "\n\nopen System\nopen System.IO\nopen JetpacFR.Core\n")
    f.write("open System.Windows\nopen System.Windows.Controls\nopen System.Windows.Media\n\n")
    f.write("\n".join(gi_modules).rstrip() + "\n")
    f.write(launcher_module)

# 3) App.fs: delete the moved ranges (high line numbers first)
del lines[jg:mw]        # JustGameWindow (replaced by GameOnlyView phase)
del lines[cheat:jg]     # CheatEngineWindow (moved)
del lines[gi:dr]        # GameImages + Projects (moved)

with io.open(APP, "w", encoding="utf-8", newline="") as f:
    f.write("\n".join(lines))

print("split complete")
print("  CheatEngineWindow.fs  <-", len(cheat_type), "lines")
print("  LauncherView.fs       <-", len(gi_modules), "lines of modules + view")
print("  GameOnlyView.fs       <- write separately (new control)")
